...
PostmanCLIReporter = function (emitter, reporterOptions, options) {
    ...
    emitter.on('done', function () {
        ...
        if (!reporterOptions.noSummary) {
            print(LF + PostmanCLIReporter.parseStatistics(run, options) + LF);
        }
...
_.assignIn(PostmanCLIReporter, {
    ...
    // @@ -265,48 +265,50 @@ _.assignIn(PostmanCLIReporter, {
    /**
     * A CLI reporter method to parse collection run statistics into a CLI table.
     *
     * @param {Object} run - The cumulative collection run object
     * @param {Object} run.stats - The cumulative collection run stats object.
     * @param {Object} run.stats.iterations - A set of values for total, pending, and failed iterations.
     * @param {Number} run.stats.iterations.total - Total iterations in the current collection run.
     * @param {Number} run.stats.iterations.pending - Pending iterations in the current collection run.
     * @param {Number} run.stats.iterations.failed - Failed iterations in the current collection run.
     * @param {Object} run.stats.requests - A set of values for total, pending, and failed requests.
     * @param {Number} run.stats.requests.total - Total requests in the current collection run.
     * @param {Number} run.stats.requests.pending - Pending requests in the current collection run.
     * @param {Number} run.stats.requests.failed - Failed requests in the current collection run.
     * @param {Object} run.stats.testScripts - A set of values for total, pending, and failed testScripts.
     * @param {Number} run.stats.testScripts.total - Total testScripts in the current collection run.
     * @param {Number} run.stats.testScripts.pending - Pending testScripts in the current collection run.
     * @param {Number} run.stats.testScripts.failed - Failed testScripts in the current collection run.
     * @param {Object} run.stats.prerequestScripts - A set of values for total, pending, and failed prerequestScripts.
     * @param {Number} run.stats.prerequestScripts.total - Total prerequestScripts in the current collection run.
     * @param {Number} run.stats.prerequestScripts.pending - Pending prerequestScripts in the current collection run.
     * @param {Number} run.stats.prerequestScripts.failed - Failed prerequestScripts in the current collection run.
     * @param {Object} run.stats.assertions - A set of values for total, pending, and failed assertions.
     * @param {Number} run.stats.assertions.total - Total assertions in the current collection run.
     * @param {Number} run.stats.assertions.pending - Pending assertions in the current collection run.
     * @param {Number} run.stats.assertions.failed - Failed assertions in the current collection run.
     * @param {Object} run.timings - A set of values for the timings of the current collection run.
     * @param {Number} run.timings.completed - The end timestamp for the current collection run.
     * @param {Number} run.timings.started - The start timestamp for the current collection run
     * @param {String} run.timings.responseAverage - The average response time across all requests
     * @param {String} run.timings.responseMin - The minimum response time across all requests
     * @param {String} run.timings.responseMax - The maximum response time across all requests
     * @param {String} run.timings.responseSd - Standard deviation of response time across all requests
     * @param {String} run.timings.dnsAverage - The average DNS lookup time of the run
     * @param {String} run.timings.dnsMin - The minimum DNS lookup time of the run
     * @param {String} run.timings.dnsMax - The maximum DNS lookup time of the run
     * @param {String} run.timings.dnsSd - Standard deviation of DNS lookup time of the run
     * @param {String} run.timings.firstByteAverage - The average first byte time of the run
     * @param {String} run.timings.firstByteMin - The minimum first byte time of the run
     * @param {String} run.timings.firstByteMax - The maximum first byte time of the run
     * @param {String} run.timings.firstByteSd - Standard deviation of first byte time of the run
     * @param {Object} run.transfers - A set of details on the network usage for the current collection run.
     * @param {String} run.transfers.responseTotal -
                The net extent of the data transfer achieved during the collection run.
    ...
    parseStatistics (run, options) {
        ...
        // @@ -319,7 +321,7 @@ _.assignIn(PostmanCLIReporter, {
        ...
        run.timings && summaryTable.push([{
            colSpan: 3,
            content: format('total run duration: %s', util.prettyms(run.timings.completed - run.timings.started)),
            hAlign: 'left' // since main style was set to right
        }]);

        // add row to show total data received
        run && run.transfers && summaryTable.push([{
            colSpan: 3,
            content: format('total data received: %s (approx)', util.filesize(run.transfers.responseTotal)),
            hAlign: 'left'
        }]);

        // add rows containing average time of different request phases
        run && run.timings && _.forEach({
            response: 'average response time:',
            dns: 'average DNS lookup time:',
            firstByte: 'average first byte time:'
        }, (value, key) => {
            run.timings[`${key}Average`] && summaryTable.push([{
                colSpan: 3,
                content: format(`${value} %s [min: %s, max: %s, s.d.: %s]`,
                    util.prettyms(run.timings[`${key}Average`]),
                    util.prettyms(run.timings[`${key}Min`]),
                    util.prettyms(run.timings[`${key}Max`]),
                    util.prettyms(run.timings[`${key}Sd`])),
                hAlign: 'left'
            }]);
        });
